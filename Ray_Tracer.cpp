...
void Scene::CalculateRightAndUp()
{
	...
	*/
	...
	double f = 1;
	double u = tan( (fov/2)*3.1415926/180 );
	double r = u * screen_x/screen_y;
	ww_tr = eye + f*forward + r*right + u*up;
	ww_tl = eye + f*forward - r*right + u*up;
	ww_br = eye + f*forward + r*right - u*up;
	ww_bl = eye + f*forward - r*right - u*up;
...
void Scene::CreateRay(int x, int y, Vector3 &ray)
{
	...
	Point3 topEdge = ww_tl + (ww_tr - ww_tl) * right_ratio;
	Point3 bottomEdge = ww_bl + (ww_br - ww_bl) * right_ratio;
	Point3 middle = bottomEdge + (topEdge - bottomEdge) * up_ratio;
...
bool RayIntersectSphere(const Point3 &eye,
	...
	//int x = 5;
	Vector3 ecs = eye - s.origin;
	double A = DotProduct(ray, ray);
	double B = 2*DotProduct(ray, ecs);
	double C = DotProduct(ecs, ecs) - s.radius * s.radius;

	double sqt = B*B - 4 * A * C;
	if (sqt <= 0)
		return false;
	
	double t1 = (-B + sqrt(sqt))/(2 * A);
	double t2 = (-B - sqrt(sqt))/(2 * A);
...
void Scene::CastRay(const Vector3 &ray,float &r,float &g,float &b)
{
	...
	if(closest_hit_sphere != -1)
	{
		...
		// Add Diffuse Light
		for (int lightnum=0; lightnum < num_point_lights; lightnum++) {
			Vector3 r1 = pl[lightnum] - closest_point;
			r1.Normalize();
			double d = 1;
			//ddiff = dotproduct(close norm)
			d = DotProduct(r1, closest_normal);
			if ( d > 0 ) {
				//contribution = d^e(specular_exponent)
				r += s[closest_hit_sphere].diffuse[0]*d;
				g += s[closest_hit_sphere].diffuse[1]*d;
				b += s[closest_hit_sphere].diffuse[2]*d;
				
				Vector3 Rr = closest_normal*2*DotProduct(r1, closest_normal)+(r1*-1);
				Rr.Normalize();
				// Add Specular light			
				double dSpec = DotProduct(Rr, ToEye);
				r += s[closest_hit_sphere].specular[0]*pow(dSpec, s[closest_hit_sphere].specular_exponent);
				g += s[closest_hit_sphere].specular[1]*pow(dSpec, s[closest_hit_sphere].specular_exponent);
				b += s[closest_hit_sphere].specular[2]*pow(dSpec, s[closest_hit_sphere].specular_exponent);
				//r = g = b = .5;
			}
			if (r>1){
				r=1; 
